...
testthat::test_that(".ipf_2d converges to target margins", {
  ...
  testthat::expect_equal(
    rowSums(result), target_rows, tolerance = 0.1
  )
  testthat::expect_equal(
    colSums(result), target_cols, tolerance = 0.1
  )
})
...
testthat::test_that(".ipf_2d handles zero rows gracefully", {
  ...
  testthat::expect_equal(
    result[2, ], c(0, 0, 0), tolerance = 0.1
  )
})
...
testthat::test_that(
  ".complete_total_trade handles multiple year-item combos",
  {
...
testthat::test_that(".get_all_country_codes returns unique sorted", {
  ...
  testthat::expect_equal(
    as.integer(result), c(1, 2, 3, 4, 5, 6)
  )
  testthat::expect_s3_class(result, "factor")
})

testthat::test_that(
  ".filter_only_items_in_cbs removes items not in cbs",
  {
    btd <- tibble::tribble(
      ~item_cbs_code, ~value,
      1, 10,
...
testthat::test_that(
  ".downscale_estimate_matrix scales rows exceeding balance",
  {
    estimates <- matrix(
      c(6, 4, 3, 7),
      nrow = 2, byrow = TRUE
    ...
    testthat::expect_equal(
      result[1, 1], 5 * 6 / 10, tolerance = 1e-6
    )
  }
)

testthat::test_that(
  ".downscale_estimate_matrix leaves small rows unchanged",
  {
    estimates <- matrix(
      c(2, 1, 3, 4),
      nrow = 2, byrow = TRUE
...
testthat::test_that(".balance_total_trade rescales larger side", {
  ...
  testthat::expect_equal(
    result$balanced_import, c(50, 100)
  )
})
