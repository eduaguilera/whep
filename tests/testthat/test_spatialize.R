...
testthat::test_that(
  "build_gridded_landuse returns one row per crop-cell with correct columns",
  {
...
testthat::test_that(
  "build_gridded_landuse conserves country totals",
  {
    fix <- two_country_fixture()
    result <- build_gridded_landuse(
      fix$country_areas,
...
testthat::test_that(
  "build_gridded_landuse distributes proportionally to pattern",
  {
    fix <- two_country_fixture()
    result <- build_gridded_landuse(
      fix$country_areas,
...
testthat::test_that(
  "build_gridded_landuse handles multiple crops",
  {
...
testthat::test_that(
  "build_gridded_landuse handles irrigation split",
  {
    country_areas <- tibble::tribble(
...
testthat::test_that(
  "build_gridded_landuse aggregates to CFTs when mapping provided",
  {
    fix <- multi_crop_fixture()
...
testthat::test_that(
  "build_gridded_landuse errors on missing columns",
  {
    ...
    testthat::expect_error(
      build_gridded_landuse(
        bad_areas, good_patterns, good_cropland, good_grid
      ),
      "item_prod_code"
    )
  }
...
testthat::test_that(
  "build_gridded_landuse handles cells with no pattern gracefully",
  {
    country_areas <- tibble::tribble(
      ~year, ~area_code, ~item_prod_code, ~harvested_area_ha,
      2000L,         1L,             15L,                100
...
testthat::test_that(
  "build_gridded_landuse handles multiple years",
  {
    country_areas <- tibble::tribble(
      ~year, ~area_code, ~item_prod_code, ~harvested_area_ha,
      2000L,         1L,             15L,               1000,
