...
testthat::test_that("get_feed_intake example returns valid tibble", {
  ...
  pointblank::expect_col_exists(
    ...
    columns = c(
      ...
      "item_cbs_code", "feed_type", "supply",
      "intake", "intake_dry_matter", "loss",
      "loss_share"
    )
  )
  pointblank::expect_col_vals_not_null(result, "year")
...
testthat::test_that("get_primary_production example returns valid tibble", {
  ...
  pointblank::expect_col_vals_in_set(
    ...
    columns = "unit",
    set = c(
      "tonnes", "ha", "t_ha", "heads", "LU", "t_head", "t_LU"
    )
  )
  pointblank::expect_col_vals_not_null(result, "value")
})
...
testthat::test_that("get_processing_coefs example returns valid tibble", {
  ...
  pointblank::expect_col_exists(
    result,
    columns = c(
      "year", "area_code", "item_cbs_code_to_process",
...
testthat::test_that("get_wide_cbs example returns valid tibble", {
  ...
  pointblank::expect_col_exists(
    ...
    columns = c(
      ...
      "domestic_supply", "food", "production",
      "feed", "seed", "import", "export",
      "other_uses", "processing", "stock_retrieval"
    )
  )
  pointblank::expect_col_vals_not_null(result, "year")
...
testthat::test_that(
  "create_n_prov_destiny example returns valid tibble",
  {
...
testthat::test_that(
  "create_n_nat_destiny example returns valid tibble",
  {
    ...
    pointblank::expect_col_exists(
      result,
      columns = c(
        "year", "item", "irrig_cat", "box",
        "origin", "destiny", "mg_n", "province_name"
      )
    )
    pointblank::expect_col_vals_not_null(result, "mg_n")
  }
...
testthat::test_that(
  "create_n_soil_inputs example returns valid tibble",
  {
    ...
    pointblank::expect_col_exists(
      ...
      columns = c(
        "year", "province_name", "item", "irrig_cat",
        "box", "deposition", "fixation", "synthetic",
        "manure", "urban"
      )
    )
    pointblank::expect_col_vals_not_null(result, "year")
...
testthat::test_that(
  "create_n_production example returns valid tibble",
  {
...
testthat::test_that(
  "calculate_nue_crops example returns valid tibble",
  {
...
testthat::test_that(
  "calculate_nue_livestock example returns valid tibble",
  {
    ...
    pointblank::expect_col_exists(
      result,
      columns = c(
        "year", "province_name", "livestock_cat",
        "item", "prod_n", "feed_n", "excretion_n",
        "nue", "mass_balance"
      )
    )
    pointblank::expect_col_vals_not_null(result, "nue")
...
testthat::test_that(
  "calculate_system_nue example returns valid tibble",
  {
