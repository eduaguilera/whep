...
- **An invariant that holds by construction cannot detect a missing
  ...
  Assert that an input was *supplied* (row count, a provenance column),
  not merely that the totals reconcile. The same shape has appeared in a
  balance check with no unit dimension (head counts balanced against
  head counts), a global mean unchanged while every cell moved, and two
  Rest-of-World buckets matching by code while covering different
  countries.
