...
- **Model the biophysics.** Gridded soil carbon dynamics (five
  ...
  and livestock feed intake and emissions —
...
from the board listed in `whep_inputs`, and the large third-party
archives (CRU, LUH2, LPJmL, HYDE, HWSD) are read from local disk through
documented `WHEP_*` environment variables, with each reader telling you
what to set. Tracing a pressure through trade is covered end-to-end in
