...
(with the HWSD soil hydraulic properties) for the soil-organic-carbon
dynamics.
...
- [`build_crop_water_consumption()`](https://eduaguilera.github.io/whep/reference/build_crop_water_consumption.md)
  : Build per-crop consumptive water from an LPJmL run, one row per
  band.
...
carbon (grassland additionally carrying grazing-excreta carbon), read
per plant functional type and as litterfall by land-use class.
...
long-format table (Task C6 final phase). The gridded soil-surface
nitrogen surplus (net inputs minus harvested-nitrogen exports) is a
balance term computed from that balance.
...
surplus-mode critical-boundary exceedance of the nitrogen surplus (see
[`calculate_n_surplus()`](https://eduaguilera.github.io/whep/reference/calculate_n_surplus.md)
under the nitrogen balance), decomposed per crop into the parts within
and above the boundary. The pathway-mode boundary instead routes each
process-based nitrogen loss to its medium-specific critical load
(ammonia to air, nitrate to the tighter of groundwater and surface
water), decomposing each medium and naming the binding boundary per
crop. The nourishment (“just”) axis supplies per-capita protein and
dietary energy, from the commodity-balance food element times the
biomass nutrition coefficients divided by population, or from FAOSTAT
Food Balance Sheet per-capita supply, and normalizes that supply onto a
piecewise adequacy score classified Under, Adequate or Over. The
boundary and nourishment sides are then crossed per crop into the 2-way
safe-and-just classification, and each crop’s exceedance,
within-boundary or total surplus nitrogen becomes the intensity of an
embodied-nitrogen trade footprint extension.
...
ratios. The LPJmL soil-cover readers supply its optional cropland and
natural-land cover inputs, and the HWSD topsoil organic-carbon reader is
its observational benchmark.
...
- [`polity_anchor_drift()`](https://eduaguilera.github.io/whep/reference/polity_anchor_drift.md)
  : Find back-cast rows whose polity label and growth proxy describe
...
- [`resolve_polity_lineage()`](https://eduaguilera.github.io/whep/reference/resolve_polity_lineage.md)
  : Resolve a national row onto the polity its grid support carries that
  year
...
Assert that an input *arrived*, never that the totals it feeds
reconcile. Zero satisfies a sum, so a consistency identity, a
conservation check and a row count are all passed by an absence.
[`check_inputs_supplied()`](https://eduaguilera.github.io/whep/reference/check_inputs_supplied.md)
refuses a column that is missing, or present and identically zero or
missing;
[`check_labels_supplied()`](https://eduaguilera.github.io/whep/reference/check_labels_supplied.md)
refuses a value vocabulary that no longer carries the labels a filter
selects on, and names what it carries instead;
[`stamp_inputs_supplied()`](https://eduaguilera.github.io/whep/reference/stamp_inputs_supplied.md)
writes the provenance label the first one reads, which no arithmetic
downstream can forge.

- [`check_inputs_supplied()`](https://eduaguilera.github.io/whep/reference/check_inputs_supplied.md)
  : Refuse an input that was never supplied
- [`check_labels_supplied()`](https://eduaguilera.github.io/whep/reference/check_labels_supplied.md)
  : Refuse a value vocabulary that no longer contains the labels a
  filter needs
- [`stamp_inputs_supplied()`](https://eduaguilera.github.io/whep/reference/stamp_inputs_supplied.md)
