...
OECD's own list of its 38 Members (oecd.org, "Members and partners",
read 2026-08-25; the URL is in the builder, which is not URL-checked,
because the host answers `R CMD check --as-cran` with a 403) and aborts
if the sheet ever stops listing a Member. Every other flagged code
already matched that list, Colombia (2020) and Costa Rica (2021)
included, and `eu27` beside it is exactly the 27 real EU members, so the
cell reads as a data-entry slip rather than a GLEAM grouping that
borrows the name. The flag is load-bearing:
[`build_energy_co2_extension()`](https://eduaguilera.github.io/whep/reference/build_energy_co2_extension.md)
reads it for two of GLEAM's three country-grouping schemes, and as
shipped it priced Comoros' meat at OECD energy intensity, up to 129
percent high (whep#574).
