...
`regions_full`, so the two tables cannot disagree. The vendored table
used to file Bhutan under `RASI` and Comoros under `RAFR` with `cbs`
`FALSE` and `fabio_code` `999`, on the grounds that neither had a
commodity balance sheet when it was compiled; that override was dropped
in whep#395, because the `faostat-cbs-new` pin now carries 175 rows for
Bhutan and 237 for Comoros, and because it left both areas'
`polity_area_code` and `reporting_polity_code` naming the country
individually while three label columns said rest-of-region. Areas WHEP
does fold, such as Andorra (`REUR`), carry `polity_area_code` 999 too.
