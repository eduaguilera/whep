...
k_source_whep <- "whep"
k_source_gadm <- "gadm"
k_source_cshapes_europe <- "cshapes_europe"
...
cshapes_europe_path <- fs::path(
  polities_inputs_path, "cshapes_europe_geometries.gpkg"
)
k_cshapes_europe <- if (file.exists(cshapes_europe_path)) {
  sf::sf_use_s2(FALSE)
