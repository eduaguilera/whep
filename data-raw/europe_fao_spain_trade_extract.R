...
# `Europe_FAO_completed.xlsx` into `data-raw/europe_fao_spain_trade.csv`.
#
# This is stage one of two. The CSV is the plain-text, diffable source of
# record; `data-raw/europe_fao_spain_trade.R` then builds the shipped package
# dataset `europe_fao_spain_trade` from it (whep#64). Only that second stage is
# offline, so only it is re-run by the data-raw freshness gate.
#
...
#     Rscript data-raw/europe_fao_spain_trade_extract.R
...
readr::write_csv(spain_trade, "data-raw/europe_fao_spain_trade.csv")
