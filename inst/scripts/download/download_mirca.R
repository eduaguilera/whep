...
download_mirca <- function(dest_dir) {
  ...
  if (!dir.exists(mirca_dir)) dir.create(mirca_dir, recursive = TRUE)

  base_url <- "https://www.geo.uni-frankfurt.de/45218031"
  crop_names <- c("01"="Wheat","02"="Maize","03"="Rice","04"="Barley","05"="Rye",
    "06"="Millet","07"="Sorghum","08"="Soybean","09"="Sunflower","10"="Potato",
    "11"="Cassava","12"="Sugar cane","13"="Sugar beet","14"="Oil palm",
    "15"="Rapeseed","16"="Groundnut","17"="Pulses","18"="Citrus","19"="Date palm",
    "20"="Grape","21"="Cotton","22"="Cocoa","23"="Coffee","24"="Others perennial",
    "25"="Fodder grasses","26"="Others annual")
  ...
  for (crop_num in names(crop_names)) {
    for (wtype in c("rainfed", "irrigated")) {
      ...
      if (file.exists(fpath)) { skipped <- skipped + 1L; next }
  ...
  if (skipped > 0) cli::cli_alert_info("MIRCA: {skipped} already existed (skipped)")
