...
if (file.exists(drainage_file)) {
  ...
  .msg(
    "  river_routing.nc: %d routed cells, %d sinks/ocean",
    sum(dr$nx_flat != -1L),
    sum(dr$nx_flat == -1L)
  )
...
if (file.exists(drainage_file) && file.exists(neighbour_file)) {
  ...
  grid_lookup[cbind(
    lon_idx_of(lon_all) + 1L,
    lat_idx_of(lat_all) + 1L
  )] <- cgi$cell
  ...
  .msg(
    "  neighbour_irrig.nc: %d land cells, %d self-assigned",
    sum(neighbour_flat != flat_all),
    sum(neighbour_flat == flat_all)
  )
...
if (file.exists(dep_file)) {
  ...
  if (has_split) {
    ...
    .msg(
      "WARNING: nhx/noy columns not found in parquet – using 50/50 fallback."
    )
  }
