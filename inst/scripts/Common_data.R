...
# Defining username path
user <- here::here("inst/extdata/input/raw/my_folder")
print(user)

# Folder to store large files. All input and output large files are stored in subfolders of this folder
L_files_path <- paste0(user,"/L_files/")
print(L_files_path)

# Folder with general coefficients and functions. Route to the location of the local copy of the General repository
General_path <- paste0(user, "/Documents_GitHub_General/") # By default, the General repository is a sub-folder of the main folder path
print(General_path)

# Load libraries ----
x <- c("tidyverse", # Data manipulation
       "data.table", # Handling large dataframes (avoids "run out of memory" error)
       "openxlsx", # Reads xlsx without Java (avoids "run out of memory" error)
       "R.utils", # General tools
       "tidyxl", # Tidying up multi-header datasets
       "unpivotr", # Tidying up multi-header datasets
       "slider", # Row-wise iterations (NOT USED???)
       "janitor", # Rows to column names
       "zoo", # Time series
       "xts", # Time series
       "sf", # Spatial objects
       "raster", #Rasters
       "rnaturalearth", #Earth layers
       "RStoolbox", #Fortifies raster objects
       "adw", # ADW interpolation
#      "rgdal", # Spatial tools (not available for this R version)
#      "maptools", # Global maps, e.g. data("wrld_simpl") (not available for this R version)
#      "Evapotranspiration",# NOT USED!
       "SPEI", # ET0 calculation
       "RODBC", # Loads SQL attribute database (used for HWSD)
       "odbc", # Reads MS Access files
#      "RSQLite", # Load SQL attribute database. NOT USED!
#      "DBI", # Load SQL attribute database. NOT USED!
       "tmap", # Plotting maps
       "viridis", # Colour graphs
       "colorspace", # Colour graphs
       "cowplot", # Arrange multiple graphs in one figure
       "lemon", # Plot features, including facet_rep_grid
       "scatterpie", # Pie charts in maps
       "ggnewscale", # Allows scaled background maps under pie charts
       "viridis", # Colour palettes
       "ggpubr", # Adds regression equations to plots
       "ggpattern", # Adds patterns to bar charts
       "patchwork", # Arranges multiple plots in one figure
       "tidyplots") # Adds error bars to plots

# lapply(x, install.packages, character.only=TRUE) #Run if needed
print("before_source")
...
ExtractSpain <- function(x) {
  utils::read.csv(paste0(L_files_path,x)) |> 
    dplyr::filter(Area=="Spain", 
                  Item.Code<2905,
                  !Item.Code%in%c(2815,2827,2556,2738,2739,2818), #Remove redundant categories
                  !Item%in%c("Sugar, Refined Equiv",
                             "Whey",
                             "Milk, Whole",
                             "Milk, Skimmed",
                             "Cheese",
    ...
    dplyr::mutate(Item=ifelse(Item=="Groundnuts (in Shell Eq)",
                              "Groundnuts",
                              Item),
...
load_cultidata <- function(){
  utils::read.csv("./output/Cultidata_1990_2020.csv")|> 
    ...
    dplyr::mutate(Prod_Mg=ifelse(Cat_1=="Fodder_green", #Express the production of fodder crops as fresh matter
    ...
    dplyr::full_join(openxlsx::read.xlsx("./input/Crops.xlsx",
                                         ...
                                         startRow = 1,
                                         check.names=TRUE) |> 
                       dplyr::rename(Area_rice=Area_ha,
                                     Prod_rice=Prod_Mg) |> 
    ...
    dplyr::mutate(Area_ha=ifelse(is.na(Area_rice),
                                 Area_ha,
                                 Area_rice),
    ...
    dplyr::mutate(Synthetic_tN=Synthetic_kgNha*Area_ha/1000,
    ...
    dplyr::mutate(Province_name=ifelse(Year==2001&Province_name=="Teruel"&Name_biomass=="Rice",
                                       "Zaragoza",
                                       Province_name))|> 
    dplyr::mutate(Grazed_ratio=Grazed_area_share/Grazed_prod_share, #In most of the cases (ca. 99%), the grazed area and production shares are the same. Therefore I ignore the area and focus on production only
...
Extract_by_ProvLU <- function(VarMap,LUMap){
  ...
  var_LU_provs <- data.frame(unlist(lapply(var_lu, FUN = function(x) { weighted.mean(x=x[,1], w=x[,2], na.rm = TRUE) })),
                             prov_Spain$COD_PROV) |> 
...
Sort_named_provinces <- function(x){
  x |> 
    ...
    dplyr::mutate(Province_name=factor(Province_name,
...
Products_from_Processing <- function(dataset_processing,dataset_aggregated){
  dataset_processing |> 
    ...
    dplyr::left_join(dataset_aggregated |>
                       dplyr::filter(Element=="Production") |> 
                       dplyr::rename(ProcessedItem=Item,
                                     Value_Coproduct=Value) |> 
                       dplyr::select(-Element)) |> 
    dplyr::mutate(Product_from_processing=Value_Product/Processing,
...
Impacts_per_FU <- function(x){
  x |> 
    dplyr::mutate(Value_ha=Value*1000/Area_ygpit_ha,
...
Names_Livestock <- openxlsx::read.xlsx("./input/Livestock.xlsx",
                                       sheet="Names_categoria_Species")

Names_Livestock_NIR <- openxlsx::read.xlsx("./input/Livestock.xlsx",
                                           sheet="Names_NIR_Species")

Names_Livestock_cat <- openxlsx::read.xlsx("./input/Livestock.xlsx",
                                           sheet="Names_cat_Species")

Names_live <- openxlsx::read.xlsx("./input/Livestock.xlsx",
                                  sheet="Names_live")

Names_poultry_rabbits <- openxlsx::read.xlsx("./input/Livestock.xlsx",
                                             sheet="Poultry_rabbits")

Names_manure_dest <- openxlsx::read.xlsx("./input/Livestock.xlsx",
                                         sheet="Names_manure_dest")

# Land use codes and coefficients (LU_type_es, LU_simple_es. GL_es, LandUse_coefs)
wb <- openxlsx::loadWorkbook("./input/LU_codes.xlsx")
...
LU_simple_es <- LU_simple_es |>
  tidyr::pivot_wider(names_from=LU_simple,
                     values_from=LU_esp)

GL_es <- GL_es|>
  tidyr::pivot_wider(names_from=Geog_Level,
                     values_from=GL_esp)

# #Country codes
Country_codes <- openxlsx::read.xlsx("./input/Country_codes.xlsx",
                                             sheet="Country_codes")
...
# Inputs keys
inputs_key <- openxlsx::read.xlsx("./input/Inputs_coefs.xlsx",
                                  sheet="Item_key",
                                  startRow = 1)  

variable_key <- openxlsx::read.xlsx("./input/Inputs_coefs.xlsx",
                                    sheet="Variable_key",
                                    startRow = 1)  


Item_eq <- openxlsx::read.xlsx("./input/Farm_Inputs.xlsx",
                               sheet="Item_Type_eq",
                               startRow = 1)

Input_type_key <- openxlsx::read.xlsx("./input/Inputs_coefs.xlsx",
                                      sheet="Input_type_key",
                                      startRow = 1)

# Grazing emission factors
Grazing_EFs <- openxlsx::read.xlsx("./input/Livestock.xlsx",
                                   sheet="Grazing_EFs",
                                   startRow = 1)

# Food waste shares
Waste_shares <- openxlsx::read.xlsx("./input/Waste.xlsx",
                                    sheet="Waste_shares",
                                    startRow = 1)
...
# Province codes
Prov_codes <- openxlsx::read.xlsx("./input/Province_Codes.xlsx",
                                  sheet="Codes") 

# Province boundaries
prov_Spain <- st_read(paste0(L_files_path,"NaturalEarth/States_Provs_shape/ne_10m_admin_1_states_provinces.shp")) |> 
  dplyr::filter(iso_a2=="ES") |> 
  dplyr::right_join(Prov_codes |> 
                      dplyr::filter(!is.na(iso_3166_2))) |> 
  dplyr::select(COD_PROV,
                geometry) 
...
prov_Spain_centroids <- data.frame(prov_Spain$COD_PROV,
                                   prov_Spain$prov_lat)
prov_Spain_coords <-data.frame(st_coordinates(prov_Spain_centroids$geometry))

prov_Spain_lat_lon<-data.frame(COD_PROV=prov_Spain$COD_PROV,
                               long=prov_Spain_coords$X,
                               lat=prov_Spain_coords$Y) |> 
  dplyr::left_join(Prov_codes) |> 
  dplyr::select(Province_name,long,lat)

prov_Spain_lat <- data.frame(prov_Spain$COD_PROV,
                             prov_Spain_coords$Y) |> 
...
Prov_codes <- prov_Spain_lat |> 
  ...
  dplyr::mutate(Province_name=as.character(Province_name),
                NAMENUTS3=as.character(NAMENUT3))

Prov_levels <- dplyr::pull(Prov_codes,
                           Province_name)
Prov_levels_all <- c("Spain",Prov_levels)

NAMENUTS3_levels <- dplyr::pull(Prov_codes,
                                NAMENUTS3)
...
Artificial_sp <- get_LU(paste0(L_files_path,"GLC_Share_GlobalLandCoverShare/glc_Artificial_shv10_01.tif"))
Cropland_sp <- get_LU(paste0(L_files_path,"GLC_Share_GlobalLandCoverShare/glc_Cropland_shv10_02.tif"))
Grassland_sp <- get_LU(paste0(L_files_path,"GLC_Share_GlobalLandCoverShare/glc_Grassland_shv10_03.tif"))
Forest_sp <- get_LU(paste0(L_files_path,"GLC_Share_GlobalLandCoverShare/glc_Trees_shv10_04.tif"))

#DRI data
DRI_ages <- openxlsx::read.xlsx("./input/Diets_coefs.xlsx",
                                sheet="DRI",
                                startRow = 1) |> 
...
Nutrients_org_conv <- openxlsx::read.xlsx("./input/Diets_coefs.xlsx",
...
Nutrients_org_conv_DM <- Nutrients_org_conv |> 
  dplyr::filter(Cat_animal=="Vegetal",
                Variable=="DM") |> 
