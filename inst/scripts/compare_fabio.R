...
.slice_year <- function(io, year) {
  row <- io[io$year == year, ]
  if (nrow(row) != 1L) stop("Year ", year, " not found in io model output.")
  list(
    Z      = row$Z[[1L]],
    Y      = row$Y[[1L]],
    X      = row$X[[1L]],
...
.print_summary <- function(label, cmp) {
  ...
  message(sprintf(
    ...
    label,
    smry$median_rel_err * 100, smry$p90_rel_err * 100,
    smry$p99_rel_err * 100,
    smry$frac_within_5pct * 100, smry$frac_within_20pct * 100
  ))
  message(sprintf(
    "  %s — FABIO: %.3e   WHEP: %.3e   rel diff: %+.1f%%",
    label, smry$total_ref, smry$total_our, smry$total_rel_diff * 100
  ))
}
...
.compare_matrix <- function(
  path, label, our_mat, fabio_labels, our_labels, yr_col,
  col_sums = FALSE
) {
  ...
  mat <- readRDS(path)
  if (is.list(mat) && !is.null(names(mat))) mat <- mat[[yr_col]]

  rs_cmp <- .compare_vectors(
    rowSums(mat), fabio_labels[, c("area_code", "item_cbs_code")],
    rowSums(our_mat), our_labels[, c("area_code", "item_cbs_code")]
  )
  ...
  if (col_sums) {
    cs_cmp <- .compare_vectors(
      colSums(mat), fabio_labels[, c("area_code", "item_cbs_code")],
      colSums(our_mat), our_labels[, c("area_code", "item_cbs_code")]
    )
...
x_cmp <- .compare_vectors(
  fabio_x,        fabio_labels[, c("area_code", "item_cbs_code")],
  our$X,          our$labels[,   c("area_code", "item_cbs_code")]
)
...
x_by_item <- x_cmp |>
  ...
  arrange(.data$ratio) |>
  select(
    "item_cbs_name", "n_areas",
    "fabio_total", "whep_total", "ratio", "median_rel_err"
  )
...
.compare_matrix(
  FABIO_Z_PATH, "Z — inter-industry flows",
  our$Z, fabio_labels, our$labels, yr_col,
  col_sums = TRUE
)
...
message(sprintf(
  ...
  max(abs(our_resid)), mean(abs(our_resid))
))
...
.compare_matrix(
  FABIO_Y_PATH, "Y — final demand",
  our$Y, fabio_labels, our$labels, yr_col
)
...
if (requireNamespace("ggplot2", quietly = TRUE)) {
  ...
  p_x <- ggplot(x_cmp, aes(x = log10(.data$ref + 1), y = log10(.data$our + 1))) +
