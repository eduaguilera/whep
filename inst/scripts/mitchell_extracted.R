# load libraries
install.packages(c("dplyr", "here", "readr", "purrr", "readxl", "ggplot2", "tidyr", "stringr"))
library(readxl)
library(dplyr)
library(tidyr)
library(ggplot2)
...
folder <- "inst/extdata/input/processed/mitchell"
...
process_sheet <- function(sheet_data) {
  ...
  # country names
  countries <- names(sheet_data)[-c(1, 2)]  # Excluir las primeras dos columnas
...
process_excel <- function(file_path) {
  ...
  all_data <- map_df(sheets, function(sheet) {
    tryCatch({
      ...
      warning(sprintf("Error procesando la hoja %s en el archivo %s: %s", 
...
for(file in excel_files) {
  tryCatch({
    ...
    warning(sprintf("Error procesando el archivo %s: %s", basename(file), e$message))
  })
}
...
crop_prod_country |>
  filter(country == "Japan", unit =="1000", year <= 1961) |>
  ggplot(aes(x = year , y = value, fill = value)) +
  geom_area(alpha = 0.6) +  
  labs(title = "Animal production area in Japan (until 1961)", y = "1000 heads") +
  facet_wrap(~item, scales = "free")


crop_prod_country |>
  filter(country %in% c("Australia", "China", "Japan", "South Africa", "Indochina"), item == "Pigs",  year <= 1961) |>
...
crop_prod_country |>
  filter(country %in% c("Australia"), item == "Wheat", unit== "1000 metric tons",  year <= 1961) |>
  ggplot(aes(x = year, y = value, fill = country)) +
  geom_area(alpha = 0.6) +  
  labs(
...
crop_prod_country |>
  filter(country %in% c("Australia"), item == "Wheat", unit== "1000 ha",  year <= 1961) |>
  ggplot(aes(x = year, y = value, fill = country)) +
  geom_area(alpha = 0.6) +  
  labs(
...
crop_prod_country |>
  filter(country %in% c("Australia", "China", "Manchuria", "Japan", "South Africa", "Iran"), item == "Barley", unit== "1000 ha",  year <= 1961) |>
  ggplot(aes(x = year, y = value, fill = country)) +
  geom_area(alpha = 0.6) +  
  labs(
    title = "Wheat Production in Australia, China, South Africa, Iran and Japan",
    x = "Year",
    y = "Production (1000 ha)"
  ) +
  theme_minimal() +
...
crop_prod_country |>
  filter(country == "Turkey", unit =="1000 metric tons", year <= 1961) |>
  ggplot(aes(x = year , y = value, fill = value)) +
  geom_area() +
  labs(title = "Crop area in Turkey", y = "1000 metric tons")+
  facet_wrap(~item, scales = "free")
