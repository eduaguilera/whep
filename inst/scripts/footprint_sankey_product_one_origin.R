...
# Usage:
#   Rscript inst/scripts/footprint_sankey_product_one_origin.R \
#     [year] [origin_area] [origin_item] [output_file]
#
# Example:
#   Rscript inst/scripts/footprint_sankey_product_one_origin.R \
#     2000 Brazil Soyabeans footprint-sankey-2000-brazil-soyabeans-product.html
#
...
.resolve_sankey_area <- function(value) {
  ...
  matched <- whep::regions_full |>
    dplyr::filter(tolower(.data$name) == tolower(value))
  if (nrow(matched) != 1) {
    stop("Could not resolve origin area `", value, "`.", call. = FALSE)
  }
  as.integer(matched$code[[1]])
}
...
paths <- compute_footprint_product_paths(
  ...
  add_area_name(
    name_column = "product_area_name",
    code_column = "product_area"
  ) |>
  add_area_name(
  ...
  dplyr::mutate(
    ...
    product = paste0(.data$product_item_name, " (", .data$product_area_name, ")"),
    final_demand_area = .data$target_area_name
  )

if (nrow(paths) == 0) {
  stop("No positive product footprint paths matched the requested origin.")
}

message("Writing product Sankey viewer to ", output_file, ".")
plot_footprint_sankey(
  ...
  title = paste("Land Footprint Product Sankey", year),
  subtitle = paste0(
    origin_item_name,
    " from ",
    origin_area_name,
    " traced to supplied final-demand products and areas."
  ),
  value_label = "ha"
)
