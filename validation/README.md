...
> **Layout & sources:** all data we compare against lives under the gitignored
> `validation/cache/` (`data/` raw sources, `ground_truth/` pinned values,
> `findings/` WHEP-side, `local_paths.json`). See **`SOURCES.md`** for the full
> manifest of every source, where it lives, and its access terms. Run the whole
...
| `gaez_potential.R` | Builds the per-country GAEZ potential cropping-intensity ceiling (local GAEZ rasters → `cache/files/GAEZ/`). |
...
| cropping_intensity | **bound** | country·crop | CROPGRIDS physical ÷ harvested | GAEZ v3 potential ceiling (local, `gaez_potential.R`) |
...
All extractors run from packaged data + the public production pin (no LPJmL).
The deterministic comparators are proven; the open piece is sourcing ground truth
for occupation/cycle/intensity (ecoinvent/GAEZ/GGCMI), which — like NASS — may
be local datasets rather than web research.
...
- WHEP is national-only, so the comparison is **sum-of-subnational vs WHEP
  ...
  what gets compared.
- `validate.R` here is a copy of the shared core (also used by
  `autoresearch/footprint_validation/`); consolidate if you keep only one.
