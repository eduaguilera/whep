...
| `WHEP_POLYCELL_SUPPORT_PATH` | For `japan_pilot.R`, the **Japan-only** polycell support: 504 polycells over 305 cells for the 46 prefecture polities, built by `inst/scripts/build_pilot_polycell_support.R` with the inland-water layer applied (land 36,995,862.69 → 36,685,203.95 ha; inland water 310,658.74 ha = 0.84%; ice 0). The registered world `polycell_support` pin has no prefecture cells **and** ships zero inland water and ice (whep#1010; the sound version is `20260825T102349Z-1a0eb`), so the pilot needs the local one and skips without it. `spatialize_grid_vintage.R` uses the same variable as a plain override. | one parquet file, path given by the env var |
