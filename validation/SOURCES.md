...
| `WHEP_POLYCELL_SUPPORT_PATH` | For `japan_pilot.R`, the **Japan-only** polycell support: 504 polycells over 305 cells for the 46 prefecture polities, built by `inst/scripts/build_pilot_polycell_support.R` with the inland-water layer applied (land 36,995,862.69 → 36,685,203.95 ha; inland water 310,658.74 ha = 0.84%; ice 0). The registered world `polycell_support` pin has no prefecture cells, so the pilot needs the local one and skips without it. (It also used to ship zero inland water and ice — whep#1010 — but whep#1012 repinned it to `20260907T111653Z-e654d`, which carries both layers; do not repin backwards onto `20260825T102349Z-1a0eb`, which predates the bucket `area_code` work of whep#907.) `spatialize_grid_vintage.R` uses the same variable as a plain override. | one parquet file, path given by the env var |
| `VAL_JP_INPUT_DIR` | For `japan_pilot.R`, a directory of prepared spatialization parquets — the same one `run_spatialize(paths = list(input_dir = ))` takes: `country_areas.parquet`, `crop_patterns.parquet`, `gridded_cropland.parquet`, `type_cropland.parquet` and optionally `multicropping.parquet`. Read through `arrow::open_dataset()` and scoped to Japan before anything is materialised, because `type_cropland.parquet` is a 268 MB world layer of which the pilot wants 305 cells. | a directory, path given by the env var |
