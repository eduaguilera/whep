...
R package developers don't use `renv`... Or do they? At least they do not seem
...
about mixing both things, but my conclusion was that it just doesn't hurt
...
Take the following code as an example, written by our colleague Justin as an
example (you do not have to understand the code, you can keep reading).
We save it in `R/sources.R`.
```r
...
again to make sure they didn't break any functionality. What if they need to change
...
Again, you don't have to understand the whole code. Just note that we use two
...
brings to actually assert things in the test's code. It's probably the most
...
In the same way, there are also ways to check whether a code is correctly
following a style or not, without explicitly changing it. In the context of
the Tidyverse style guide, these two points directly match with two R packages:

- `styler`: it applies the Tidyverse style guide to a specific code, be it a chunk,
a file or an entire project. Most code editors incorporate some way of doing this.
Since you are most likely using RStudio, you can do it
...
sure this check passes, since it will also be automatically checked on your
Pull Requests (see the [next section](#automatic-checks-on-pull-requests) on this
guide). The check we will use is:
```r
lintr::lint_package(
