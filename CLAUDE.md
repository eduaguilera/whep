...
- **Regenerating the LPJmL-derived pins after a new run**: four pins carry LPJmL model *output* — `lpjml-grass-availability`, `lpjml-grass-productivity`, `lpjml-grass-natural-net-c`, `lpjml-soc-hydrology` (the last two arrive with the SOC pin work). Regenerate them **together, from one run**, with the single entry point `regenerate_whep_lpjml_pins()` in the `~/whep_inputs` project — dry-run by default (prints a manifest plus the change against each pin it would replace), `upload = TRUE` to publish. Refreshing only some of them leaves WHEP mixing two LPJmL versions across its feed, soil-carbon and water chains at once, which is worse than consistently using either version and is invisible downstream because every pin still loads with the right schema. The six `lpjml-wind-*` / `lpjml-rsds-*` / `lpjml-rlds-*` pins are climate **forcing** (they feed *into* LPJmL, so they do not change with the model version) — those come from `inst/scripts/prepare_spatialize_all.R`, never from the regeneration above.
- **LPJmL output variable names are version-dependent.** 6.x renames some outputs to their CF short names — `mprec.nc` holds `pr` where 5.x held `prec`. Readers resolve the name against what the file actually contains (`.hydro_var_aliases()`), because a run directory carries no version stamp and both versions' output can sit side by side. When a new rename appears, add it there; the reader aborts listing the file's actual variables rather than failing with a NULL lookup.
