...
- **LPJmL outputs are the one input a user cannot obtain**, so unlike the
  ...
  the pinned and run-derived paths cannot silently disagree. Regenerate and
  re-upload them from a new run with
  `upload_whep_lpjml_soc_artifacts()` in the `~/whep_inputs` project.
