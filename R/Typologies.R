#' Typologies
#'
create_typologies_grafs_spain <- function(make_map = TRUE, shapefile_path = "C:/PhD/GRAFS/Production Boxes/Final Files/Inputs/ne_10m_admin_1_states_provinces.shp", map_year = 1980) {
  inputs_dir <- "C:/PhD/GRAFS/Production Boxes/Final Files/Inputs"

  # Load datasets
  data <- .load_inputs(inputs_dir, shapefile_path)

  data$sf_provinces$name <- stri_trans_general(data$sf_provinces$name, "Latin-ASCII")
  data$Livestock_Prod_ygps$Province_name <- stri_trans_general(data$Livestock_Prod_ygps$Province_name, "Latin-ASCII")
  ...
  data$sf_provinces <- data$sf_provinces[!data$sf_provinces$name %in% c("Las_Palmas", "Tenerife"), ]
  ...
  feed_domestic_prov <- .calculate_feed_domestic_supply(data$GRAFS_Prod_Destiny_git, lu_aggregated)

  # Calculate feed import per province based on national imports & LU shares
  feed_import_by_province <- .calculate_feed_import_share(data$PIE_FullDestinies_FM, lu_aggregated)

  # Calculate imported feed share at province level
  feed_imported_share <- .calculate_imported_feed_share(feed_import_by_province, feed_domestic_prov)


  # Assign Typologies + map
  typologies_result <- .assign_typologies_decision_tree(
    livestock_density,
...
.aggregate_cropland_productivity <- function(npp_df) {
  cropland_prod <- npp_df |>
    ...
    dplyr::mutate(Productivity_kgN_ha = Prod_MgN_total / Area_ha_cropland * 1000) |>
    dplyr::arrange(Year, Province_name)
...
.calculate_feed_domestic_supply <- function(grafs_df, lu_df) {
...
.calculate_feed_import_share <- function(feed_df, lu_df) {
  feed_filtered <- feed_df |>
    ...
    dplyr::group_by(Year) |>
    dplyr::summarise(Total_feed_import = sum(Value_destiny, na.rm = TRUE), .groups = "drop")
  ...
  return(feed_import_by_province)
}

# Calculate feed share of imported/consumed feed ---------------------------------------------------------------------
.calculate_imported_feed_share <- function(feed_import_by_province, domestic_feed_by_province) {
  feed_import_by_province |>
    dplyr::left_join(domestic_feed_by_province, by = c("Year", "Province_name")) |>
    dplyr::mutate(
      Total_feed_MgN = Domestic_feed_MgN + Feed_import_MgN,
      Imported_feed_share = Feed_import_MgN / Total_feed_MgN,
      Imported_feed_share = ifelse(is.nan(Imported_feed_share), NA, Imported_feed_share)
    ) |>
    dplyr::select(
      Year,
      Province_name,
...
.assign_typologies_decision_tree <- function(
  ...
  typologies <- livestock_density |>
    ...
    dplyr::mutate(
      Typologie = dplyr::case_when(
        Livestock_density < 0.3 & Productivity_kgN_ha > 60 ~ "Specialized cropping system",
        Livestock_density < 0.3 & Productivity_kgN_ha <= 60 ~ "Extensive cropping system",
        Livestock_density >= 0.3 & Semi_nat_share > 0.6 ~ "Extensive mixed crop-livestock system",
        Livestock_density >= 0.3 & Semi_nat_share <= 0.6 & Imported_feed_share < 0.5 ~ "Intensive mixed crop-livestock system",
        Livestock_density >= 0.3 & Semi_nat_share <= 0.6 & Imported_feed_share >= 0.5 ~ "Specialized livestock-farming system",
        TRUE ~ NA_character_
      )
    ) |>
