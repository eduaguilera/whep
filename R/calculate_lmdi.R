...
#' @family calculations
#'
#' @importFrom dplyr filter select mutate summarise group_by ungroup across
#'   all_of bind_rows distinct n_distinct arrange if_else first n
#' @importFrom rlang syms .data
#' @importFrom stringr str_extract_all str_detect str_match
...
calculate_lmdi <- function(
  ...
  # Helper function: parse identity string
  ...
  # Helper function: extract selectors from identity
  ...
  # Helper function: filter by group
  ...
  # Helper function: evaluate factor expression
  ...
  if (!is.null(rolling_mean)) {
    ...
    if (nrow(data) == 0) {
      warning(
        "No data remaining after applying rolling mean. ",
        "Consider using a smaller rolling_mean value."
      )
    }
  ...
  # Auto-detect selectors from identity
    ...
    for (i in seq_len(nrow(periods))) {
      ...
      if (nrow(d0) == 0 || nrow(d_final) == 0) {
        if (verbose) {
          ...
          message(sprintf(
            "Skipping period %s-%s for %s: No data found",
            t0,
            t_final,
            group_tag
          ))
