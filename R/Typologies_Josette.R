...
#'
create_typologies_of_Josette <- function(make_map = TRUE, shapefile_path = "C:/PhD/GRAFS/Production Boxes/Final Files/Inputs/ne_10m_admin_1_states_provinces.shp", map_year = 2020) {
  inputs_dir <- "C:/PhD/GRAFS/Production Boxes/Final Files/Inputs"

  # Load datasets
  data <- .load_inputs_typologies_josette(inputs_dir, shapefile_path)

  consumption <- .calculate_consumption_and_prod(data$GRAFS_Prod_Destiny_git)
  food_consumption_df <- consumption$food_consumption
  production_df <- consumption$production

  crop_feed <- .calculate_crop_prod_and_feed_ingestion(data$GRAFS_Prod_Destiny_git)
  cropland_prod_df <- crop_feed$cropland_prod
  animal_ingestion_df <- crop_feed$animal_ingestion

  intensive_list <- .calculate_intensive_imported_feed(
  ...
  seminatural_df <- .calculate_seminatural_feed_share(data$GRAFS_Prod_Destiny_git)
  feed_domestic_df <- .calculate_feed_domestic_share(data$PIE_FullDestinies_FM, intensive_list$lu_totals)
  manure_share_df <- .calculate_cropland_manure_share(data$N_inputs)
  ...
  typologies_df <- food_consumption_df |>
    ...
    dplyr::left_join(intensive_list$livestock_density_df, by = c("Year", "Province_name")) |>
    dplyr::left_join(intensive_list$imported_feed_share_df, by = c("Year", "Province_name")) |>
  ...
  return(typologies_df)
}


# Load input datasets -------------------------------------------------------------------------------------------------------------------
.load_inputs_typologies_josette <- function(inputs_dir, shapefile_path) {
...
#'
.calculate_consumption_and_prod <- function(GRAFS_Prod_Destiny_git) {
  # Food consumption
  food_consumption <- GRAFS_Prod_Destiny_git |>
    dplyr::filter(Destiny == "Food") |>
    dplyr::group_by(Year, Province_name) |>
    dplyr::summarise(Food_Consumption_MgN = sum(MgN, na.rm = TRUE), .groups = "drop")

  # Agricultural production
  production <- GRAFS_Prod_Destiny_git |>
    dplyr::filter(Destiny %in% c("Food", "Feed", "Other_uses", "Export", "Import")) |>
    dplyr::group_by(Year, Province_name, Destiny) |>
    dplyr::summarise(MgN = sum(MgN, na.rm = TRUE), .groups = "drop") |>
    dplyr::group_by(Year, Province_name) |>
    dplyr::summarise(
      Production_MgN = sum(MgN[Destiny %in% c("Food", "Feed", "Other_uses", "Export")], na.rm = TRUE) -
...
.calculate_crop_prod_and_feed_ingestion <- function(GRAFS_Prod_Destiny_git) {
  # Cropland  production
  cropland_prod <- GRAFS_Prod_Destiny_git |>
    dplyr::filter(Box == "Cropland", Destiny %in% c("Food", "Feed", "Other_uses", "Export", "Import")) |>
    dplyr::group_by(Year, Province_name, Destiny) |>
    dplyr::summarise(MgN = sum(MgN, na.rm = TRUE), .groups = "drop") |>
    tidyr::pivot_wider(names_from = Destiny, values_from = MgN, values_fill = 0) |>
    dplyr::mutate(
  ...
  animal_ingestion <- GRAFS_Prod_Destiny_git |>
    ...
    dplyr::group_by(Year, Province_name) |>
    dplyr::summarise(Animal_Ingestion_MgN = sum(MgN, na.rm = TRUE), .groups = "drop")
...
#'
.calculate_intensive_imported_feed <- function(livestock_df, codes_coefs_df, npp_df, feed_df, destiny_df) {
  ...
  domestic_feed_by_province <- destiny_df |>
    ...
    dplyr::group_by(Year, Province_name) |>
    dplyr::summarise(Domestic_feed_MgN = sum(MgN, na.rm = TRUE), .groups = "drop")

  imported_feed_share_df <- .calculate_imported_feed_share(feed_import_by_province, domestic_feed_by_province)
...
#'
.calculate_seminatural_feed_share <- function(destiny_df) {
  seminatural_feed <- destiny_df |>
    dplyr::filter(Destiny == "Feed", Box == "Semi_natural_agroecosystems") |>
    dplyr::group_by(Year, Province_name) |>
    dplyr::summarise(SemiNatural_feed_MgN = sum(MgN, na.rm = TRUE), .groups = "drop")
...
.calculate_feed_domestic_share <- function(feed_df, lu_df) {
  # Filter relevant feed data: Production, Exports and Imports destined for feed use
  feed_summary <- feed_df |>
    dplyr::filter(Element %in% c("Production", "Export", "Import"), Destiny == "Feed") |>
    tidyr::pivot_wider(
      names_from = Element,
  ...
  total_lu_spain <- lu_df |>
    dplyr::group_by(Year) |>
    dplyr::summarise(LU_total_spain = sum(LU_total, na.rm = TRUE), .groups = "drop")
  ...
  feed_prov <- lu_with_share |>
    ...
    dplyr::mutate(
      ...
      # Calculate local feed share per province: share of feed coming from domestic sources
      local_feed_share = (Production_prov - Export_prov) / ((Production_prov - Export_prov) + Import_prov),
    ) |>
    dplyr::select(
...
#'
.calculate_cropland_manure_share <- function(n_input_df) {
  cropland_n_inputs <- n_input_df |>
    dplyr::filter(Box == "Cropland") |>
    dplyr::group_by(Year, Province_name) |>
    dplyr::summarise(
      MgN_manure = sum(MgN_manure, na.rm = TRUE),
      MgN_total = sum(MgN_dep + MgN_fix + MgN_syn + MgN_manure + MgN_urban, na.rm = TRUE),
      .groups = "drop"
    ) |>
    dplyr::mutate(
  ...
  df |>
    dplyr::mutate(
      Typology = dplyr::case_when(
        ...
        Cropland_Production_MgN > 1.5 * Animal_Ingestion_MgN ~ "Specialized stockless cropping system",
        Livestock_density > 1 & Imported_feed_share > 0.33 ~ "Specialized livestock system",
        SemiNatural_feed_share > 0.5 ~ "Grass-based crop & livestock system",
        local_feed_share > 0.05 & Manure_share > 0.05 ~ "Forage-based crop & livestock system",
...
#' Create map
#'
.create_typologies_map_josette <- function(typologies_df, shapefile_path, map_year) {
  layer_name <- tools::file_path_sans_ext(basename(shapefile_path))
