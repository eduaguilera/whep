...
#' - `"luh2"` (default): permanent grassland area (item_cbs 3000, LUH2 pasture
#'   and rangeland) taken from [build_primary_production()]. This shares the
#'   gridded LUH2 land-use basis used by the crop land extensions and by
#'   livestock spatialisation. Rotational fallow (item_cbs 3003) is excluded
#'   because the `cropgrids_fallow` crop extension already attributes fallow to
#'   crops, so counting it here too would double count it.
