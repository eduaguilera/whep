#' Plots: N Inputs, Production and NUE in Spain
#'
#' @description
#' This code is creating Plots visualizing nitrogen (N) inputs (deposition, fixation, synthetic, urban, manure)
#' and N production in Spain between 1860 and 2020 for the GRAFS model on a provincial level as well as NUE for cropland and semi natural agroecosystems
#'
...
create_plots_n_inputs_grafs_spain <- function() {
  ...
  plot_cropland_provinces <- .plot_n_inputs_production_provinces(data$GRAFS_Prod_Destiny, data$N_Inputs_combined)

  #' Return list
  list(
    cropland_semi_natural = plot_cropland_semi_natural_agroecosystems,
    nue_spain = plot_nue_spain,
    cropland = plot_cropland,
    provinces = plot_cropland_provinces
  )
}
...
.load_inputs_plots <- function(inputs_dir) {
  list(
    ...
    NUE = readr::read_csv(file.path(inputs_dir, "NUE.csv"))
  )
}
