...
#'   `method_recycling_n`, `method_synthetic`, `method_deposition_scope`,
#'   `method_unattributed`. At `resolution = "polity"`: `area_code`,
#'   `item_cbs_code`, `year`, `fert_type`, `method_recycling_n`,
#'   `method_synthetic`, `method_deposition_scope`, `method_unattributed`,
#'   `n_input_t` (summed over cells).
...
#
# The "exclude" policy is whep#532's original behaviour, kept selectable so the
# tonnage it removes can be measured by differencing rather than inferred: it
# is the only branch that does not conserve mass, and it says so out loud.
.ni_allocate_unattributed <- function(inputs, data) {
  ...
  if (nrow(unattributed) == 0L) {
    return(.ni_stamp_unattributed(inputs, method))
  }
  .ni_report_unattributed(unattributed, method)
  if (method == "exclude") {
    kept <- dplyr::filter(inputs, !is.na(.data$item_cbs_code))
    return(.ni_stamp_unattributed(kept, method))
  }
  support <- .ni_allocation_support(.ni_land_support(data), method)
  ...
  .ni_check_unallocated(sourced, allocated, support, method)
  dplyr::bind_rows(
    dplyr::filter(inputs, !is.na(.data$item_cbs_code)),
    dplyr::select(allocated, dplyr::all_of(.ni_source_schema()))
  ) |>
...
.ni_check_unallocated <- function(sourced, allocated, support, method) {
