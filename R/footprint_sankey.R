...
.sankey_group_small_nodes <- function(
  ...
  for (i in seq_along(stage_cols)) {
    ...
    other <- other_label[i]
    # Rank every node on its own value, including one genuinely labelled
    # `other` (e.g. "Meat, Other" in FAO-style nomenclatures) — it must
    # compete for a keep-slot like any other node, not be forced into the
    # synthetic small-node bucket purely because its label collides with the
    # bucket's display label (#214).
    keep <- paths |>
...
.sankey_js <- function() {
  paste(
    c(
      ...
      "function groupVisiblePaths(paths){let grouped = paths.map(p => ({ ...p })); model.stages.forEach(stage => { const rawMax = state.maxNodes[stage.key]; const maxNodes = rawMax == null ? Infinity : Number(rawMax); if (!Number.isFinite(maxNodes)) return; const n = Math.max(1, Math.floor(maxNodes)); const other = stage.other_label || model.options.other_label || 'Other'; const totals = new Map(); grouped.forEach(p => totals.set(p[stage.key], (totals.get(p[stage.key]) || 0) + Number(p.value || 0))); const keep = new Set([...totals.entries()].sort((a,b) => b[1] - a[1] || String(a[0]).localeCompare(String(b[0]))).slice(0, n).map(([name]) => name)); grouped.forEach(p => { if (p[stage.key] !== other && !keep.has(p[stage.key])) p[stage.key] = other; }); }); return aggregatePaths(grouped);}",
