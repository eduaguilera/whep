...
#
# End to end on the live pins, `population_source = "pin_wpp_fbs_fallback"` at
# the defaults, the three shapes plus area 351 remove 1,186 rows of 51,362 and
# the world sum falls 23.2% at 1961, 17.7% at 2000 and 15.6% at 2021 -- it was
# 30.2%, 21.5% and 18.4% too high. On `"pin_wpp_fallback"` 1,045 rows go and
# 1961 falls 7.6%; on the default `"pin"` 143 rows go and the affected years
# 1850-1992 were 0.19%-0.58% too high.
...
#'     is the finer grain, and it does not reproduce the federation's own
#'     figure: on `"pin_wpp_fbs_fallback"` the successor sum falls 1.6% short of
#'     the USSR at 1961 and runs 2.0% over it at 1991, and falls 14.8% (1961) to
#'     17.5% (1991) short of the Yugoslav SFR — the Kosovo gap of #863. Before
#'     1950 no source has the successors at all, so the federation's population
#'     is lost rather than redistributed. It also removes three areas from the
#'     table outright, because they exist only in the overlapping years:
#'     51 Czechoslovakia, and — undoing exactly what the FBS fill is for —
#'     186 Serbia and Montenegro (#862) and 151 Netherlands Antilles (#787).
...
#'   [resolve_polity_lineage()] does on the spatial side (#1004, #1114): its
#'   default `basis = "historical_polity"` places a 1961 row keyed on area 185
...
#'   reporter, exactly as `"successors"` does here. The two are complementary,
#'   not competing: that function says which polity's **cells** a row belongs
#'   on and moves no value, while this one says which of two **rows** for one
#'   territory survives. The package therefore has one convention, stated in
#'   both places and selectable in both.
