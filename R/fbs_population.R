...
# 18.4% in that year. Measured on the live pins it is the only area with a
# bucket and no polity -- 63 (area, year) rows, 67 before the vintage pick,
# 1961-2023 -- its value is EXACTLY the sum of its four members in 64 of those
# 67 raw rows, off by ten persons in the other three (2015, 2016 and 2020 in the
# new pin, which publishes the item in thousands), and all four members are
# present in every year it appears, so the row can only ever be a duplicate.
...
.fbs_pop_report_aggregates <- function(resolved) {
  ...
  cli::cli_inform(c(
    i = "Dropped {length(codes)} FAOSTAT area{?s} that reach{?es/} a
         ...
         {.val {codes}}.",
    i = "{cli::qty(length(codes))}{?It is a statistical aggregate/They are
         statistical aggregates} over areas that report separately, so summing
         {cli::qty(length(codes))}{?it/them} into a population would double
         count every member."
  ))
  invisible(NULL)
}
