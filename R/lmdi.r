...
#'
#' @description
#'   This function performs Logarithmic Mean Divisia Index (LMDI) decomposition
#'   with flexible identity parsing and auto-detection of grouping variables.
#'   It supports both additive and multiplicative decomposition forms with
#'   automatic panel balancing and zero/NA treatment following Ang (2015)
#'   methodology.
#'
#' @param data A data frame containing the variables for decomposition.
#' @param identity A character string specifying the decomposition identity in
#'   the format 'target:factor1*factor2*...'. Factors can include selectors
#'   like 'variable[group]' or ratios like 'var1/var2'.
#' @param identity_labels An optional character vector of labels for the target
#'   and factors. Must have length equal to 1 + number of factors.
#' @param time_var A character string specifying the time variable column name.
#'   Default is "year".
#' @param periods A data frame, matrix, or vector specifying time periods for
#'   decomposition. If NULL, consecutive periods are automatically generated.
#' @param periods_2 An optional additional set of periods to combine with the
#'   main periods parameter.
#' @param analysis_by A character vector of column names to group the analysis
#'   by. Can be combined with auto-detected selectors from the identity.
#' @param rolling_mean An optional numeric value >= 2 specifying the window
#'   size for centered rolling mean smoothing of variables.
#' @param output_format A character string specifying output format. Either
#'   "clean" (default) or "total" for full details.
#' @param verbose A logical value indicating whether to print detailed progress
#'   messages. Default is TRUE.
#'
#' @return
#'   A tibble containing LMDI decomposition results with columns for period,
#'   factor labels, component types, and additive/multiplicative contributions.
#'   Additional attributes include the original identity, period targets, and
#'   analysis grouping variables.
#'
#' @export
#'
#' @examples
#' # Basic LMDI decomposition
#' library(dplyr)
#'
#' # Create sample data
#' sample_data <- tibble::tribble(
#'   ~year, ~country, ~sector, ~energy, ~activity, ~intensity,
#'   2000, "A", "industry", 100, 50, 2.0,
#'   2010, "A", "industry", 120, 60, 2.0,
#'   2000, "A", "transport", 80, 40, 2.0,
#'   2010, "A", "transport", 100, 50, 2.0
#' )
#'
#' # Perform decomposition
#' result <- lmdi_calculation(
#'   data = sample_data,
#'   identity = "energy:activity*intensity",
#'   analysis_by = "sector",
#'   periods = c(2000, 2010),
#'   verbose = FALSE
#' )
...
lmdi_calculation <- function(
  ...
  # Extract selectors from identity (e.g., [sector], [sector+fuel])
  ...
  if (!is.null(rolling_mean)) {
    if (!is.numeric(rolling_mean) || rolling_mean < 2) {
      stop(
        ...
        "(number of years for centered moving average)"
      )
    }
      ...
      if (!requireNamespace("data.table", quietly = TRUE)) {
        ...
        data <- data |>
          tidyr::complete(
            ...
            fill = as.list(stats::setNames(
              rep(NA_real_, length(numeric_vars)),
              numeric_vars
            ))
          )
      } else {
        ...
        complete_grid <- do.call(
          data.table::CJ,
          c(unique_vals, list(sorted = FALSE))
        )
    ...
    if (verbose && n_added > 0) {
      ...
      message(sprintf(
        "  - Structure: %d groups × %d years = %d expected rows",
        n_groups_before,
        n_years,
        n_groups_before * n_years
      ))
    }
    ...
    n_zeros <- sum(sapply(data[numeric_vars], function(x) {
      sum(x == 0, na.rm = TRUE)
    }))
    ...
    remaining_zeros <- sum(sapply(data[numeric_vars], function(x) {
      sum(x == 0, na.rm = TRUE)
    }))
    ...
    if (min_years < k_orig) {
      ...
      if (verbose) {
        message(sprintf(
          "rolling_mean=%d is larger than available years in some groups (min=%d). Using k=%d.",
          k_orig,
          min_years,
          k_eff
        ))
    ...
    if (verbose) {
      message(sprintf(
        "Applying %d-year centered rolling mean to variables: %s",
        k_eff,
    ...
    if (nrow(data) == 0) {
      warning(
        "No data remaining after applying rolling mean. Proceeding without smoothing. Consider using a smaller rolling_mean value."
      )
    ...
    if (verbose) {
      message(sprintf(
        "Data reduced to %d rows after rolling mean (removed edge years with NA)",
        nrow(data)
      ))
    }
  }
  ...
  if (!is.null(identity_labels)) {
    if (length(identity_labels) != length(factors) + 1) {
      stop(sprintf(
        "identity_labels must have %d elements: 1 for target + %d for factors. Got %d.",
        length(factors) + 1,
        length(factors),
        length(identity_labels)
      ))
    }
  ...
  if (!is.null(analysis_by)) {
    ...
    if (verbose && length(selectors_detected) > 0) {
      message(sprintf(
        "Auto-detected selectors from identity: %s",
    ...
    if (verbose) {
      message(sprintf(
        "Auto-detected group_vars from selectors: %s",
        paste(group_vars, collapse = ", ")
      ))
    }
  }
  ...
  if (is.null(periods)) {
    ...
    if (length(years) < 2) {
      stop("Need at least two periods to perform the decomposition")
    }
    ...
    if (is.matrix(periods) || is.data.frame(periods)) {
      ...
      if (length(periods) == 2) {
        ...
        periods <- data.frame(t0 = periods[-length(periods)], tT = periods[-1])
      } else {
  ...
  for (idx in seq_len(nrow(analysis_groups))) {
    ...
    for (i in seq_len(nrow(periods))) {
      ...
      if (nrow(d0) == 0 || nrow(dT) == 0) {
        if (verbose) {
          group_tag <- if (is.null(analysis_values)) {
            ...
            paste(
              sprintf("%s=%s", names(analysis_values), analysis_values),
              collapse = ", "
            )
          }
          message(sprintf(
          ...
          if (verbose) {
            ...
            if (!is.null(group_vals) && length(group_vals) > 0) {
              message(sprintf(
                "  Group values: %s",
                paste(
                  sprintf("%s=%s", names(group_vals), group_vals),
                  collapse = ", "
                )
              ))
            }
      ...
      factor_rows <- data.frame(
        ...
        additive_annual = if (period_years > 0) {
          period_contribs_add / period_years
        } else {
          period_contribs_add
        },
        multiplicative = period_contribs_mult,
        multiplicative_annual = if (period_years > 0) {
          period_contribs_mult^(1 / period_years)
        } else {
          period_contribs_mult
        },
        multiplicative_log = period_contribs_mult_log_vals,
        multiplicative_log_annual = if (period_years > 0) {
          period_contribs_mult_log_vals / period_years
        } else {
          period_contribs_mult_log_vals
        },
      ...
      target_row <- data.frame(
        ...
        additive_annual = if (period_years > 0) {
          total_change / period_years
        } else {
          total_change
        },
        multiplicative = target_ratio,
        multiplicative_annual = if (
          period_years > 0 && !is.na(target_ratio) && target_ratio > 0
        ) {
          target_ratio^(1 / period_years)
        } else {
          target_ratio
        },
        ...
        multiplicative_log_annual = if (
          period_years > 0 && !is.na(target_ratio) && target_ratio > 0
        ) {
          log(target_ratio) / period_years
        } else {
          ifelse(
            !is.na(target_ratio) && target_ratio > 0,
            log(target_ratio),
      ...
      period_results[[length(period_results) + 1]] <- dplyr::bind_rows(
        factor_rows,
        target_row
      )
    ...
    closure_summary <- analysis_result |>
      ...
      dplyr::summarise(
        target_add = dplyr::first(.data$additive[
          .data$component_type == "target"
        ]),
        sum_factors_add = sum(
          .data$additive[.data$component_type == "factor"],
          na.rm = TRUE
        ),
        target_mult = dplyr::first(.data$multiplicative[
          .data$component_type == "target"
        ]),
        prod_factors_mult = {
