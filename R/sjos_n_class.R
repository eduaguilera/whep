...
#' country (see [build_n_boundary_exceedance()]). The cell comparison is made
#' before any crop attribution and the overshoot is not capped at the actual
#' pressure. With `negative_critical = "keep"`, a cell whose critical surplus
#' is negative exceeds even at zero actual surplus, its overshoot is larger
#' than its actual surplus, and its `within_boundary_n_t` is negative; with
#' `"clamp"` that cell's allowance is zero instead. A crop whose own surplus
#' is negative inside an exceeding cell receives a negative exceedance share,
#' so it falls on the `"Within_boundary"` side unless positive shares from
#' other cells of the same country outweigh it.
#'
